...
class Team:
    ...
    def process_team(self, res, team_name):
        """
        Use BS4 to get, from the given mlb.com page, all anchor tag lines for players in the team.
        Then use them to call Player.get_player_info for geocoding/etc.
        Return list of Player objects, and an int for number of them not mappable.
        """
        player_pages_not_reached = 0
        ...
        for anch in player_anchors:
            ...
            try:
                ...
                soup = bs4.BeautifulSoup(res.text, "html.parser")
                player, null_hometown, not_geocoded = player.get_player_info(
                    player, soup)
                player_list.append(player)
            except requests.exceptions.RequestException as err:
                write_log_and_or_console(f"ERROR:  {player.player_name}\'s page could not be located.")
...
class Player:
    ...
    def get_player_info(self, player, soup):
        ...
        write_log_and_or_console(f"NAME & POSITION:  {self.player_name} ({self.position})")
        home_town_output = f"HOME TOWN:        {self.hometown}"
        if self.hometown_lat != -1 and self.hometown_lat != -1:
            home_town_output += " (" + str(self.hometown_lat) + ", " + \
                                str(self.hometown_long) + ")"
        write_log_and_or_console(home_town_output)

        if (self.hometown_lat == -1 and self.hometown_lat == -1) and \
                self.hometown != "\n\tNULL (no hometown found on page)":
...
def make_gmplot_and_kml(players, team_code, team_color, num_missing):
    ...
    for player in players:
        if player.hometown_lat != -1 and player.hometown_long != -1:
            ...
            pnt = kml.newpoint(name=player.player_name + " (" + player.position + ") - " +
    ...
    filename = f"MLB_player_hometowns_{team_code.upper()}_{START_TIME.strftime('%Y%m%d_%H%M')}"
    if num_missing > 0:
